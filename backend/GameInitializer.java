	...
	protected void populateGameMap(GamePiece[][] gameMap, int numBombs){
		...
		for (int i = 0; i < gameMap.length; i++){
		    for (int j = 0; j < gameMap[0].length; j++){
		        gameMap[i][j] = new EmptyLand(); // Initialize grid to contain EmptyLands
		    }
		}
    	...
    	while(totalBombs != 0){
		    ...
		    if (!gameMap[randomColumn][randomRow].isBomb()){
		    	gameMap[randomColumn][randomRow] = new Bomb(); // Set bomb in a random spot on map
	    		totalBombs--;
	    		countSurroundings(gameMap, randomColumn, randomRow, gameMap.length, gameMap[0].length);
	    		// Update EmptyLand to count surrounding mines
		    }
        } // End while
	}
